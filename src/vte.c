...
vte_sequence_handler_change_cursor_color (VteTerminal *terminal, GValueArray *params)
{
	...
	if (params != NULL && params->n_values > 0) {
		...
		if (vte_parse_color (name, &color)) vte_terminal_set_color_V2(terminal, VTE_CUR_BG, color.red, color.green, color.blue);
			...
			glong palette_size)
{
	...
	for (i=0; i < G_N_ELEMENTS(terminal->pvt->palette); i++) {
		...
		if ((glong) i < palette_size) color = palette[i];

		/* Set up the color entry. */
		vte_terminal_set_color_V2(terminal, i, color.red, color.green, color.blue);
	}
...
void vte_terminal_set_color_V2(VteTerminal *this, int which, int red, int green, int blue) {
	...
	PangoColor *_color = &this->pvt->palette[which];
		if ((_color->red == red) && (_color->green == green) && (_color->blue == blue)) return;
		_color->red = red; _color->green = green; _color->blue = blue;
		
	if (gtk_widget_get_realized((GtkWidget *)this) != TRUE) return;

	/* If we're setting the background color, set the background color on the widget as well. */
	if (which == VTE_DEF_BG) vte_terminal_queue_background_update(this);

	/* and redraw */
	if (which == VTE_CUR_BG) _vte_invalidate_cursor_once(this, FALSE);
	else _vte_invalidate_all(this);
}
