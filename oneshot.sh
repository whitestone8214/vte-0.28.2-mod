...
if (test "$1" = "build"); then
	#gcc $(pkg-config --cflags --libs gtk+-x11-2.0 gio-unix-2.0) -I. -Wno-deprecated-declarations -fPIC src/{caps,debug,iso2022,marshal,matcher,pty,reaper,ring,vte,vteconv,vtedraw,vteseq,vtetc}.c -shared -o libvte.so || exit 1
	gcc $(pkg-config --cflags --libs gtk+-x11-2.0 gio-unix-2.0) -I. -Wno-deprecated-declarations -fPIC src/{caps,debug,iso2022,marshal,matcher,pty,reaper,ring,vte,vteconv,vtedraw,vteseq}.c -shared -o libvte.so || exit 1
